...
function Matching({ user, setHasLocation }) {
  ...
  return (
    ...
    >
      ...
      {images.length > 0 && (
        <div style={{ width: 320, height: 270, background: '#f6f6f6', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden', padding: 0, margin: '0 auto', borderTopLeftRadius: 22, borderTopRightRadius: 22 }}>
          <img
            src={images[imgIdx]}
            alt={item.category}
            loading="lazy"
  ...
  {/* Info row: emoji, category, color, size, distributed horizontally */}
      ...
      {showDetails && (
        <div style={{ width: '100%', padding: '16px 18px 0 18px', fontSize: 15, color: '#444', display: 'flex', flexDirection: 'column', gap: 8, minHeight: 100 }}>
          {item.brand && <div><strong>{t('brand')}:</strong> {item.brand}</div>}
          {item.material && <div><strong>{t('material')}:</strong> {item.material}</div>}
          {item.sizeDetails && <div><strong>{t('size_details')}:</strong> {item.sizeDetails}</div>}
          {item.additionalInfo && <div><strong>{t('info')}:</strong> {item.additionalInfo}</div>}
        </div>
      )}
